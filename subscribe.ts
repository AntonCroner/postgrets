...
function tuples(x, columns, xi, transform) {
	...
	for (let i = 0; i < columns.length; i++) {
		...
		transform.raw
			? (row[i] = transform.raw === true ? value : transform.value.from ? transform.value.from(value, column) : value)
...
function parseEvent(x: string) {
	...
	return (
		(command || "*") + (path ? ":" + (path.indexOf(".") === -1 ? "public." + path : path) : "") + (key ? "=" + key : "")
	)
}
