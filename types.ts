...
export const types = {
	string: { to: 25, from: null, /* defaults to string */ serialize: x => "" + x },
	...
	boolean: { to: 16, from: 16, serialize: x => (x === true ? "t" : "f"), parse: x => x === "t" },
	date: {
		to: 1184,
		from: [1082, 1114, 1184],
		...
		types.push(
			x instanceof Parameter
				? (parameters.push(x.value),
				  x.array ? x.array[x.type || inferType(x.value)] || x.type || firstIsString(x.value) : x.type)
...
function valuesBuilder(first, parameters, types, columns, options) {
	return first
		.map(
			row =>
				"(" + columns.map(column => stringifyValue("values", row[column], parameters, types, options)).join(",") + ")"
		)
		.join(",")
}
